// StatBooks Telegram bot — runs INSIDE the backend process (webhook mode) so a
// single Render service hosts both the API and the bot. mountWebhook(app) wires
// the Telegram webhook route onto the shared Express app.
...
// Talk to our own API in-process (no public round-trip).
const BACKEND        = (process.env.BOT_BACKEND_URL || `http://127.0.0.1:${process.env.PORT || 10000}`).replace(/\/$/, '');
...
// ── Bot instance (webhook mode; updates arrive via the shared Express app) ─────
const bot = new TelegramBot(TOKEN);
// Secret token Telegram echoes back so we can verify webhook calls.
const WEBHOOK_SECRET = require('crypto').createHash('sha256').update(TOKEN).digest('hex').slice(0, 32);
...
// Mount the Telegram webhook route on the shared Express app and register the
// webhook URL with Telegram. Called from app.js (only when a token is set).
function mountWebhook(app) {
  app.post(WEBHOOK_PATH, (req, res) => {
    if (req.get('X-Telegram-Bot-Api-Secret-Token') !== WEBHOOK_SECRET) return res.sendStatus(403);
    try { bot.processUpdate(req.body); } catch (e) { console.error('[bot] processUpdate:', e.message); }
    res.sendStatus(200);
  });

  const base = process.env.RENDER_EXTERNAL_URL || process.env.WEBHOOK_URL;
  if (base) {
    bot.setWebHook(`${base.replace(/\/$/, '')}${WEBHOOK_PATH}`, { secret_token: WEBHOOK_SECRET })
      .then(() => console.log('[bot] webhook registered at', base + WEBHOOK_PATH))
      .catch((e) => console.error('[bot] setWebHook failed:', e.message));
  } else {
    console.warn('[bot] No public URL (RENDER_EXTERNAL_URL/WEBHOOK_URL) — webhook not registered');
  }
}
