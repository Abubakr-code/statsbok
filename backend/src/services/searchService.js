 ...
 *
 * By explicit product decision the search never reads books/quotes from the
 * database. Every result is generated by AI (OpenRouter) so ANY quote, author
 * or topic can be identified — not only what happens to be stored locally.
...
async function searchQuotesEnhanced(input, lang = 'en') {
  ...
  list = list.map((item) => localizeResultByLang(item, effectiveLang));

  // For Uzbek/Russian queries, drop any book the AI tagged as another language
  // so an English title never shows up for an Uzbek quote.
  if (effectiveLang === 'uz' || effectiveLang === 'ru') {
    const langFiltered = list.filter((r) => {
      const b = r.book || {};
      return !b.language || b.language === effectiveLang;
    });
    if (langFiltered.length > 0) list = langFiltered;
  }

  const finalResults = list.slice(0, Math.max(RESULT_LIMIT, 20));
