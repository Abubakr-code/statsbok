...
const { strictAiSearch } = require('./strictAiSearchService');
...
Your task: Given a search query (author name, exact quote, topic, genre, or book title), return only books you know are real and directly relevant.
...
5. Return at most 5 books. Never fill a quota and never add merely similar books to an exact-source query.
...
REQUIRED JSON fields for each book:
{
  ...
  const userMsg = `Search for books related to: "${query}". Return only directly verified real books (maximum 5). Respond in ${langName}.`;
  const messages = [
...
/**
 * Main search endpoint — AI-ONLY.
 *
 * The active search path never reads books/quotes from the database. A first
 * model proposes only known sources and an independent second model must
 * approve the exact title + author. Uncertain results are withheld instead of
 * being replaced with random related books.
 */
async function searchQuotesEnhanced(input, lang = 'en', diag = null) {
  ...
  const strictResult = await withTimeout(
    strictAiSearch(raw, effectiveLang, diag),
    23000
  );
  const finalResults = strictResult?.results || [];
...
1. Use only exact sentences/quotes you confidently know are from the actual book. Never invent, paraphrase, or imitate the author's style. If uncertain, return an empty JSON array.
